...
public class ConsultaPersona implements WindowListener, ActionListener
{
	...
	TextArea txtPersonas = new TextArea(5,20);
	Label lblPersonas = new Label("PERSONAS");
	Button btnPDF =  new Button ("Exportar a PDF");
	//Añadimos objetos necesarios para PDF
	public static final String DEST = "ConsultaPersona.pdf";
	PdfFont font;
	PdfWriter writer;
	...
	public ConsultaPersona(int tipoUsuario)
	{
		this.tipoUsuario=tipoUsuario;
		//Configuramos la pantalla y añadimos los Listeners
		ventana.setLayout(new FlowLayout());
		ventana.setSize(270,200);//aplicamos ancho y altura
		ventana.addWindowListener(this);
		// Conectar con la BD
		connection = bd.conectar();
		ventana.add(lblPersonas);
		ventana.add(txtPersonas);
		txtPersonas.setText(bd.ConsultaPersona(tipoUsuario));
	...
	public void actionPerformed(ActionEvent evento)
	{
		...
		if(evento.getSource().equals(btnPDF))
		{
			...
			try
			{
				...
				document.add(new Paragraph("Personas de la empresa:"));
				// Create a PdfFont
				try
				{
				...
				try
				{
					...
					while(resultSet.next())
						...
						document.add(new Paragraph((resultSet.getInt("idPersona")+
